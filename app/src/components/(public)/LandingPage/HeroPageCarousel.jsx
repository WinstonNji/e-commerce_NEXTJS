          ...
          carouselContent.map((product, i) => (
            <div key={i} className={`absolute flex flex-col px-8 ${i === index ? 'opacity-100' : 'opacity-0'} duration-1000 ease-in-out w-full h-full transition-opacity  md:flex-row-reverse md:items-center md:pl-12 lg:px-32  `}>
              ...
              <div className='flex flex-col gap-2  w-full flex-1 md:gap-5'>
                {/* Caption */}
                <p className='text-white underline decoration-accent underline-offset-4 font-semibold'>{product.captionText}</p>
                {/* Main text */}
                <p className='font-semibold text-xl md:text-2xl lg:text-3xl'>{product.mainText}</p>
                {/* Button */}
                <div>
                  <Link className='btn btn-accent text-white w-fit hover:bg-accent-hover hover:px-12 transition-all duration-150 ease-in-out' href={product.navigationLink}>
                    
                      {product.navigationLinkText}
